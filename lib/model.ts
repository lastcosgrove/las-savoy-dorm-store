import {schoolDate} from './dates';
export type Role = 'staff' | 'prefect';
export type Product = {id:string;name:string;category:string;price_cents:number;cost_cents?:number;image_url:string;sort_order:number;active:boolean;low_stock_threshold:number};
...
export type Line = {product_id:string;product_name:string;quantity:number;unit_price_cents:number;unit_cost_cents?:number;line_total_cents:number};
...
export type Movement = {id:string;product_id:string;quantity_delta:number;movement_type:'delivery'|'sale'|'void'|'count_correction';unit_cost_cents?:number;transaction_id?:string;operator_id:string;note:string;occurred_at:string};
...
export function chargesCsv(sales:Sale[]){return [['Date (Europe/Zurich)','Timestamp (UTC)','Student number','Student','Items','Total CHF','Operator','Transaction ID','Reverses','Note'],...sales.map(s=>[schoolDate(s.occurred_at),s.occurred_at,s.student_number,s.student_name,s.items.map(i=>`${i.quantity} × ${i.product_name}`).join('; '),s.total_cents/100,s.operator_name,s.id,s.original_transaction_id||'',s.note||''])].map(row=>row.map((value,index)=>index===5&&typeof value==='number'?'\"'+value.toFixed(2)+'\"':csvCell(value)).join(',')).join('\r\n')}
