-- Reviewed production foundation. Apply only to the selected EU project.
...
create policy roster_read on public.students for select to authenticated using(private.actor_id() is not null);
create policy products_read on public.products for select to authenticated using(private.actor_id() is not null);
create policy operators_read on public.operators for select to authenticated using(private.actor_id() is not null);
create policy prices_read on public.price_history for select to authenticated using(private.actor_id() is not null);
create policy stock_read on public.stock_movements for select to authenticated using(private.actor_id() is not null);
create policy shifts_read on public.shifts for select to authenticated using(operator_id=private.actor_id() or private.is_staff());
...
create function public.open_shift(p_id uuid,p_opened_at timestamptz,p_device_id uuid) returns uuid language plpgsql security definer set search_path='' as $$
 ...
 if found then if prior.operator_id<>actor or prior.device_id<>p_device_id then raise exception 'Shift conflict';end if;return p_id;end if;
 if p_opened_at>now()+interval '5 minutes' then raise exception 'Invalid start time';end if;
...
create function public.close_shift(p_id uuid,p_closed_at timestamptz) returns void language plpgsql security definer set search_path='' as $$
 begin
 if private.actor_id() is null then raise exception 'Operator access required';end if;
 if p_closed_at>now()+interval '5 minutes' then raise exception 'Invalid end time';end if;
...
create function public.ingest_sale(p jsonb) returns uuid language plpgsql security definer set search_path='' as $$
 ...
 begin
 if actor is null or (p->>'operator_id')::uuid<>actor then raise exception 'Operator mismatch';end if;
 perform pg_advisory_xact_lock(hashtextextended(sid::text,0));
 select * into prior from public.transactions where id=sid;
 if found then if prior.source_payload<>p then raise exception 'Idempotency conflict';end if;return sid;end if;
 select * into s from public.shifts where id=(p->>'shift_id')::uuid;
 if s.id is null or s.operator_id<>actor or s.device_id<>(p->>'device_id')::uuid then raise exception 'Invalid shift';end if;
 if occurred<s.opened_at or (s.closed_at is not null and occurred>s.closed_at) or occurred>now()+interval '5 minutes' then raise exception 'Invalid sale time';end if;
 ...
 if qty not between 1 and 1000 or price<0 then raise exception 'Invalid quantity or price';end if;
 ...
 if expected_price is null or price<>expected_price or row_item->>'product_name'<>expected_name then raise exception 'Catalog mismatch; staff review required';end if;
 if (row_item->>'line_total_cents')::bigint<>qty::bigint*price then raise exception 'Line total mismatch';end if;
 total:=total+qty::bigint*price;
 end loop;
 if total<>(p->>'total_cents')::bigint or total>2147483647 then raise exception 'Total mismatch';end if;
...
create function public.void_sale(p_id uuid,p_original uuid,p_shift uuid,p_reason text) returns uuid language plpgsql security definer set search_path='' as $$
 declare original public.transactions;s public.shifts;actor uuid:=private.actor_id();begin
 if not private.is_staff() or length(trim(p_reason))=0 then raise exception 'Staff and reason required';end if;
 ...
 if s.id is null or s.closed_at is not null then raise exception 'Open shift required';end if;
...
create function public.staff_transactions() returns setof public.transactions language plpgsql security definer set search_path='' as $$
